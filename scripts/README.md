...
기반 라이브러리 7개와 공식 후속 바이너리 8개의 빌드 pin은 `inventory/dependencies.lock.json`에 별도로 둡니다.
...
PFM/Puzzles Lib의 공개팩용 공식 파일은 `inventory/published-artifacts.lock.json`에 고정하고
`release_pack.py`가 필요할 때 복원한다. 소스 recipe와 운영 기준본을 바꾸지 않으며
공식 URL·SHA-256·SHA-512·내장 고지와 최종 의존성을 검증한다.
