...
## Q. 왜 Automobility와 Automobility-Refueled 저장소가 둘 다 있었나요?
...
| Minefed에서의 역할 | 과거 1.20.4 백포트 자료, 현재 서브모듈 제거 | 관측한 운영 JAR의 소스 및 현재 빌드 입력 |
...
Refueled의 역할과 고정 커밋은 [mods.lock.json](../inventory/mods.lock.json)에 기록되어 있다. 기존 `Automobility`는 `sourceRepositories`에서도 제거했으며, 운영 JAR 목록에는 `automobility-refueled`만 있다. [빌드 recipe](../inventory/build-recipes.json)도 `Automobility-Refueled`의 `:fabric:remapJar`만 호출한다.
...
따라서 원본과 Refueled를 둘 다 설치할 대상으로 취급하면 안 된다. 과거 두 저장소를 함께 관리한 것은 두 JAR를 모드팩에 넣기 위한 구성이 아니었다.
...
제거 전 조사한 기존 `Automobility`의 `gradle.properties`는 Minecraft `1.20.4`로 바뀌었지만, `fabric/src/main/resources/fabric.mod.json`은 여전히 Minecraft `1.21.x`와 Java 21을 요구했다. 버전 숫자만 낮췄다고 1.20.4 이식이 완료된 것은 아니다.

조사한 소스 이력은 다음과 같다. 제거한 Automobility는 종료 저장소 기록에서, 활성 Refueled는 lock 파일에서 전체 커밋과 브랜치를 확인할 수 있다.
...
기존 Automobility는 운영 빌드에 필요하지 않아 소스 관리를 종료했다. 과거 URL과 커밋 기록은 당시 조사 근거이며, 원격 삭제 후에도 해당 URL에서 소스를 내려받을 수 있다는 뜻은 아니다. 실제 기능을 옮길 필요가 생기면 필요한 원저자 소스를 확보해 변경별로 검토하고 이식한다.

## Q. 모드 ID가 다르면 리소스팩과 월드 데이터도 완전히 분리되어 있나요?

**아니다. Fabric의 모드 ID와 게임 리소스의 namespace를 구분해야 한다.**

제거 전 조사한 기존 소스의 `InitlessConstants.java`와 현재 [Refueled 상수](../Automobility-Refueled/common/src/main/java/io/github/foundationgames/automobility/util/InitlessConstants.java)는 모두 내부 namespace를 `automobility`로 정의한다. Java 패키지와 Fabric 진입점도 `io.github.foundationgames.automobility`를 공유한다.
