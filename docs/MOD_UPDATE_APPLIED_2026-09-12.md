...
아래는 버전 갱신 당시의 선택·검증 기록이다. 이후 전체 JAR 포함 요청으로 현재 릴리즈는
서버 67개·클라이언트 65개 모드를 모두 직접 내장한다. 과거 `download`/`manual` 표시는
배포 판단 이력이며, 현재 파일 포함 여부는 동봉 manifest의 `archiveIncluded`와
[릴리즈 안내](RELEASING.md)를 따른다. Modern Lights 2.5.0도 양쪽에 내장된다.
