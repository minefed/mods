...
이번 전환 대상은 Fabric API, Architectury, Botarium, Lavender, owo, Resourceful Config, ResourcefulLib다. 각 소스의 조사 기준 커밋과 Minefed 관리 커밋 사이 변경이 `AGENTS.md`뿐임을 확인했다. 7개 gitlink와 소스 빌드 recipe를 제거하고 공식 바이너리 recipe로 전환했다. 후속 정리에서는 사용자 요청에 따라 이 7개의 Minefed 원격 fork도 삭제했다. 기존 로컬 checkout은 Git에서 무시하는 경로에 과거 이력으로 남기며, 빌드와 업데이트는 공식 배포본 및 원저자 소스를 사용한다. 원격 삭제 상태와 과거 fork의 URL·브랜치·전체 커밋은 [종료 저장소 기록](../inventory/retired-repositories.json)에 남긴다. 모드팩 입력은 68개를 유지하고, 소스 빌드는 53개에서 46개로 줄었다.
