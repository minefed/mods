...
현재 구성은 소스 빌드 47개와 바이너리 25개다. 기반 라이브러리 7개, 공식 후속 바이너리 8개와 Continuity·MCEF·Sodium·Indium, 총 19개는 recipe의
...
5. `verifyModpackResources`가 모든 선택 JAR의 모델·블록 상태·레시피·전리품 JSON 문법을 검사한다.
   ...
   기준은 `inventory/resource-baseline.json`에 원본 SHA-256과 리소스 개수로 기록한다.
   CI에 과거 소스 모드 JAR가 없어도 같은 검사를 수행하며, 로컬 원본이 있으면 해시와
   개수를 재확인한다. 기준 해시가 맞지 않거나 기록이 빠졌으면 검사를 생략하지 않고 실패한다.
