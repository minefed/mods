...
- **서로 다른 빌드 도구:** 원본 Gradle wrapper/JDK 요구사항을 보존했다. 기존 루트 composite build에 모든 저장소를 단순 추가하면 서로 다른 Gradle 플러그인과 버전이 충돌할 수 있다. 모드별 wrapper와 명시적인 최종 Fabric JAR 경로를 사용하는 빌드 설계가 필요하다.
- **기존 루트 구성:** 기존 `build.gradle`은 minefed-display 빌드 의존성이 빠져 있고, 파일명 패턴으로 결과 JAR를 수집한다. 루트 `gradle.properties`에는 오래된 MTR 버전 `4.0.0-prerelease.1`도 남아 있다. 현재 일괄 빌드 완료로 간주하지 않는다.
...
- **소스와 바이너리 차이:** CityCraft와 Macaw Doors/Fences의 공개 소스는 운영 Fabric JAR의 직접적인 빌드 입력이 아니다. Paladin Furniture, Modern Lights 및 일부 라이브러리도 관리 소스 버전과 운영 버전이 다르므로 기준 JAR를 유지한다.
...
- **커밋 수 기반 버전:** AGENTS.md만 추가해도 `git describe`/커밋 수로 버전을 계산하는 프로젝트의 출력 버전이 바뀔 수 있다. 예를 들어 Chunky 기준 소스는 `1.3.0-146-gd3956d6`이며 지침 커밋 뒤에는 커밋 수가 증가한다. 후속 빌드는 버전 입력을 명시하거나 별도 기준 소스 worktree를 사용해야 한다.
...
- **외부 개발 환경 설정:** Yuushya Modelling의 원본 Gradle 설정에는 개발자 로컬 프록시가 들어 있다. 빌드 시 해당 설정을 검토해야 한다.
