...
초기 조사에서 MTR와 기존 Automobility의 gitlink는 이전 등록 커밋보다 최신 소스로 갱신했다. 당시 display와 webstreamer는 이미 최신 기준 커밋이었다. Chisels-and-Bits는 조직에 있던 fork를 새로 서브모듈에 등록했다. 표는 초기 조사 기준이며, 이후 지침과 빌드 수정을 포함한 현재 관리 커밋은 lock의 `source.commit`과 gitlink에서 확인한다.
...
- **기존 Automobility:** 1.20.4-backport의 properties와 달리 Fabric 메타데이터는 1.21.x/Java 21을 요구한다. 운영 파일은 별개인 Automobility Refueled 0.4.3.b이며 독립 서브모듈로 고정했다. 두 저장소는 대체 포크이며 원본은 빌드 입력이 아니다. 분리 이유와 변경 시 확인 사항은 [Automobility Q&A](AUTOMOBILITY.md)를 참고한다.
